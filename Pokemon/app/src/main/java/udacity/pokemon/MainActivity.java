    ...
    private class GetPokemon extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... arg0) {
            ...
            try {
                ...
                Log.v("pokemon1", jsonString);


            } catch (IOException e) {
                Log.e("GetPokemon", "Error at doInBackground Http Call");
                return null;
            }
            ...
            if (jsonString != null) {
                try {
                    ...
                    JSONObject myRootJSONObject = new JSONObject(jsonString);
                    // TODO: Get the JSON Array node and name it "pokemons"
                    JSONArray pokemons = myRootJSONObject.getJSONArray("pokemon");

                    // looping through all Contacts
                    for (int i = 0; i < pokemons.length(); i++) {
                        //TODO: get the JSONObject and its three attributes
                        JSONObject myPokemon = pokemons.getJSONObject(i);
                        String name = myPokemon.optString("name").toString();
                        String candy = myPokemon.optString("candy").toString();
                        String id = String.valueOf(myPokemon.optInt("id"));
